...
class Ackley:
    ...
    def calc(self, x):
        ...
        self.v = (
            -self.a * np.exp(-self.b * np.sqrt(s1 / n))
            - np.exp(s2 / n)
            + self.a
            + np.exp(1)
        )
...
class Eggholder:
    ...
    def calc(self):
        self.f = -(
            (self.z + 47) * np.sin(np.sqrt(abs(self.z + self.y / 2 + 47)))
...
class Levy:
    def calc(self, x):
        n = len(x)
        self.f = 10 * n + np.sum(x ** 2 - 10 * np.cos(2.0 * np.pi * x))
...
class Michalewicz:
    def calc(self, x):
        ...
        n = len(x)
        j = np.arange(1.0, n + 1)
        self.f = -np.sum(np.sin(x) * np.sin(j * x ** 2 / np.pi) ** (2.0 * m))
