...
class Michalewicz:
    ...
    def SysEq(self):
        ...
        n = len(self.x)
        j = np.arange(1., n+1)
        self.f = -np.sum(np.sin(self.x)*np.sin(j*self.x**2/np.pi)**(2.*michalewicz_m))


OptMichalewicz = OptimizationProblem(Michalewicz)
OptMichalewicz.Primal = "SysEq"
OptMichalewicz.x = "x"
OptMichalewicz.x0 = np.ones(2,)*+0
OptMichalewicz.xL = np.ones(2,)*-5
OptMichalewicz.xU = np.ones(2,)*+5
OptMichalewicz.f = "f"
