...
#    El3.initialize()
    ...
    def velocityMag(self):
        ...
        q = np.abs(self.q)
        self.MF = (1+(2/np.pi)*(MFMax-1) *
                 np.arctan((np.pi/2)*(MFSlp/(MFMax-1)*np.abs(self.qd))))
    ...
    def residual(self):
        self.velocityMag()
        self.R = self.m*self.qdd + self.d*self.qd + self.MF*self.k*self.q - self.F
...
simulation1.run()
plt.figure()
plt.plot(simulation1.tAll, simulation1.q,
...
plt.show()
plt.figure()
plt.plot(simulation1.tAll, simulation1.qd/1000/1000*60*60,
         label=simulation1.PrimalNonlinearSolver)
#plt.title('Velocity')
...
plt.show()
plt.figure()
plt.plot(simulation1.tAll, simulation1.qdd/1000,
         label=simulation1.PrimalNonlinearSolver)
...
plt.show()
plt.figure()
plt.plot(simulation1.q, -1*simulation1.F,
         label=simulation1.PrimalNonlinearSolver)
#plt.title('Acceleration')
...
plt.show()
plt.figure()
plt.plot(simulation1.tAll, simulation1.qNabla[:, 0],
          label=simulation1.PrimalNonlinearSolver)
#plt.title('Position sensitivity')
#plt.title('position sensitivity $\\nabla q$ [$\partial$ mm / $\partial x$]')
plt.xlabel("time $t$ [s]")
plt.legend(Model.DesVarLabels, frameon=False, loc='center left',
            bbox_to_anchor=(1, 0.5))
sns.despine()
plt.show()
plt.figure()
plt.plot(simulation1.tAll, simulation1.qdNabla[:, 0]/1000/1000*60*60,
          label=simulation1.PrimalNonlinearSolver)
...
plt.show()
plt.figure()
plt.plot(simulation1.tAll, simulation1.qddNabla[:, 0]/1000,
          label=simulation1.PrimalNonlinearSolver)
#plt.title('Acceleration sensitivity')
plt.ylabel('acceleration sensitivity $\\nabla\\ddot{q}$  [$\\partial$ m/s² / $\\partial x$]')
plt.xlabel("time $t$ [s]")
plt.legend(Model.DesVarLabels, frameon=False, loc='center left',
            bbox_to_anchor=(1, 0.5))
sns.despine()
