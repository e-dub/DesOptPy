...
def BandConParabolicFn(omega, omegaBands, c=2e-5, norm=False):
    ...
    for jj in range(np.size(omega)):
        for ii in range(np.shape(omegaBands)[0]):
            if norm:
                g[ig] = c*(omega[jj]/omegaBands[ii][0]-1) * \
                          (1-omega[jj]/omegaBands[ii][1])
            else:
                g[ig] = c*(omegaBands[ii][0]-omega[jj]) * \
...
def ConSens(omega, domegadx, omegaL, omegaU, c):
    return(-c*omegaU*(-1+omega/omegaL)*domegadx/omega**2 +
...
def BandConParabolicSensFn(omega, domegadx, omegaBands, c=2e-5, norm=False):
    ...
    for jj in range(np.size(omega)):
        for ii in range(np.shape(omegaBands)[0]):
            if norm:
                dgdx[ig] = ConSens(omega[jj], domegadx[jj], omegaBands[ii, 0],
                                   omegaBands[ii, 0],
                                   (omegaBands[ii, 1]+omegaBands[ii, 0])/2.)
            else:
                dgdx[ig] = c*omegaBands[ii][0]*domegadx[ii] + \
        ...
        for ii in range(np.shape(omegaBands)[0]):
            if norm:
                if omega[jj] < omegaBands[ii][1]:
                    g[ig] = (omega[jj]/omegaBands[ii][0]-1)*omegaBands[ii][0]/(omegaBands[ii][1]-omegaBands[ii][0])
                else:
                    g[ig] = (1-omega[jj]/omegaBands[ii][1])*omegaBands[ii][0]/(omegaBands[ii][1]-omegaBands[ii][0])
            else:
                if omega[jj] < omegaBands[ii][1]:
...
def BandConBallisticFn(omega, omegaBands, a=1e-8, b=0, norm=True,
                       infFilter=False):
    g = np.zeros((np.shape(omegaBands)[0]*np.size(omega)))
    ig = 0
    if np.size(omega) == 1:
    ...
    for jj in range(np.size(omega)):
        for ii in range(np.shape(omegaBands)[0]):
            c = (-1-2*b)/(omegaBands[ii][1]-omegaBands[ii][0]) * \
                (lambertw(-1/np.exp(1+a), -1)-lambertw(-1/np.exp(1+a), 0)).real
            d = 1/c*(-lambertw(-1/np.exp(1+a), b)-(1+a)).real
            if norm:
                g[ig] = 1/a*((1+a)+c*(omega[jj]+d-omegaBands[ii][0]) -
                             np.exp(c*(omega[jj]+d-omegaBands[ii][0])))
            else:
                g[ig] = (1+a)+c*(omega[jj]+d-omegaBands[ii][0]) - \
                              np.exp(c*(omega[jj]+d-omegaBands[ii][0]))
            ig += 1
            if infFilter:
...
def BandConBallisticSensFn(omega, domegadx, omegaBands, a=1e-8, b=0,
                           norm=True, infFilter=False):
    ...
    for jj in range(np.size(omega)):
        for ii in range(np.shape(omegaBands)[0]):
            ...
            if norm:
                ...
                dgdx[ig, :] = c*domegadx[jj]*(1-np.exp(c*(omega[jj]+d -
                                                       omegaBands[ii][0])))
            ig += 1
    if infFilter:
