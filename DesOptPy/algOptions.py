...
class setDefault:
    def __init__(self, Alg):
        ...
        if Alg == "MMA":
            ...
            self.RHOB = 0.0  # BFGS-Update Matrix Initialization Parameter
            ...
            self.iprint = (
                2  # Output Level (0- None, 1- Final, 2- Major, 3- Major Details)
            )
            ...
            self.maxoutiter = (
                2e3  # Maximum Number of Outer Loop Iterations (Major Iterations)
            )
            self.maxinniter = (
                2e2  # Maximum Number of Inner Loop Iterations (Minor Iterations)
            )
            self.stopcriteria = 1  # Stopping Criteria Flag
            self.stopiters = (
                10  # Consecutively Number of Outer Iterations for convergence
            )
            ...
            self.scaling = (
                1  # Design Variables Scaling (0- no scaling, 1- scaling [-1,1])
            )
            self.IFILE = False
        elif Alg == "MIDACO":
            self.ACC = 1e-3
...
def setUserOptions(UserOpt, Alg, OptName, OptAlg):
    if Alg[:5] == "PyGMO":
        ...
        UserOpt.IFILE is True
    ):  # needs to be changed as all non-pygmo algorithms land here! only for pyOpt!
    ...
    if Alg == "MMA":
        ...
        OptAlg.setOption(
            "RHOB", UserOpt.RHOB
        )  # BFGS-Update Matrix Initialization Parameter
        OptAlg.setOption(
            "MODE", UserOpt.MODE
        )  # NLPQL Mode (0 - Normal Execution, 1 to 18 - See Manual)
        OptAlg.setOption(
            "LQL", UserOpt.LQL
        )  # QP Solver (True - Quasi-Newton, False - Cholesky)
    elif Alg == "IPOPT":
        ...
        OptAlg.setOption(
            "nf_max", UserOpt.nf_max
        )  # Maximum Number of Function Evaluations
        OptAlg.setOption(
            "iprint", UserOpt.iprint
        )  # Print Flag (<0-None, 0-Final, 1,2-Iteration)
        ...
        OptAlg = pg.sms_emoa(
            hv_algorithm=UserOpt.hv_algorithm,
            gen=UserOpt.gen,
            cr=UserOpt.cr,
            eta_c=UserOpt.eta_c,
            m=UserOpt.m,
        ...
        OptAlg = pg.bee_colony(gen=UserOpt.gen, limit=UserOpt.limit)
    elif Alg == "PyGMO_ms":
        OptAlg = pg.ms(algorithm=UserOpt.algorithm, iter=UserOpt.iter)
    elif Alg == "PyGMO_mbh":
        OptAlg = pg.mbh(
            algorithm=UserOpt.algorithm,
            stop=UserOpt.stop,
            perturb=UserOpt.perturb,
            screen_output=UserOpt.screen_output,
        )
    elif Alg == "PyGMO_cstrs_co_evolution":
        OptAlg = pg.cstrs_co_evolution(
            original_algo=UserOpt.original_algo,
        ...
        OptAlg = pg.scipy_fmin(
            maxiter=UserOpt.maxiter,
            xtol=UserOpt.xtol,
            ftol=UserOpt.ftol,
            maxfun=UserOpt.maxfun,
            disp=UserOpt.disp,
        ...
        OptAlg = pg.scipy_cobyla(
            max_fun=UserOpt.max_fun,
            rho_end=UserOpt.rho_end,
            screen_output=UserOpt.screen_output,
        )
    elif Alg == "PyGMO_nlopt_cobyla":
        OptAlg = pg.nlopt_cobyla(
            max_iter=UserOpt.max_iter, ftol=UserOpt.ftol, xtol=UserOpt.xtol
        )
    elif Alg == "PyGMO_nlopt_bobyqa":
        OptAlg = pg.nlopt_bobyqa(
            max_iter=UserOpt.max_iter, ftol=UserOpt.ftol, xtol=UserOpt.xtol
        )
    elif Alg == "PyGMO_nlopt_sbplx":
        OptAlg = pg.nlopt_sbplx(
            max_iter=UserOpt.max_iter, ftol=UserOpt.ftol, xtol=UserOpt.xtol
        )
        ...
        OptAlg = pg.nlopt_auglag(
            aux_algo_id=UserOpt.aux_algo_id,
            max_iter=UserOpt.max_iter,
            ftol=UserOpt.ftol,
            xtol=UserOpt.xtol,
