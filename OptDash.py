...
xConv.add_tools(HoverTool(tooltips=[('Iteration','@x'),('Value', '@y')], mode='vline'))
...
for i in range(len(x[0])):
    xNormConv.line(it, x[:, i], legend_label=xNormLabels[i],
                   color=colorList[i], line_width=LineWidth)
...
xNormConv.add_tools(HoverTool(tooltips=[('Iteration','@x'),('Value', '@y')], mode='vline'))

fgMaxConv = figure(plot_width=wConvPlot, plot_height=hConvPlot)
fgMaxConv.line(it, f, legend_label="f", color=colorObjective,
               line_width=LineWidth)
...
fgMaxConv.extra_y_ranges = {"gMax": Range1d(start=min(np.min(gMax), -np.max(np.abs(g))/100),
...
fgMaxConv.line(it, gMax, legend_label=gMaxLabel, y_range_name="gMax",
               color=colorConstraint, line_width=LineWidth)
...
fgMaxConv.add_tools(HoverTool(tooltips=[('Iteration','@x'),('Value', '@y')], mode='vline'))
...
gConv.add_tools(HoverTool(tooltips=[('Iteration','@x'),
                                    ('Value', '@y')],  mode='vline'))
# Bar diagrams
...
fgBar.extra_y_ranges = {"gBar": Range1d(start=-np.max(np.abs(g))*1.1,
                                        end=np.max(np.max(np.abs(g))*1.1))}
...
fgBar.vbar(top=g[0], x=gLabels, width=0.9, y_range_name="gBar",
           color="darkgrey")
