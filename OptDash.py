...
for i in range(len(x[0])):
    if i < 9:
        ...
        xNormLabels.append(
            'x̂{0}'.format(chr(0x2081)) + '{0}'.format(chr(0x2080))
        )
        ...
        xLabels.append(
            'x{0}'.format(chr(0x2080 + int(str(i)[0]) + 1))
            + '{0}'.format(chr(0x2080))
        )
        xNormLabels.append(
            'x̂{0}'.format(chr(0x2080 + int(str(i)[0]) + 1))
            + '{0}'.format(chr(0x2080))
        )
...
for i in range(np.size(g, axis=1)):
    if i < 9:
        ...
        gLabels.append(
            'g{0}'.format(chr(0x2080 + int(str(i)[0]) + 1))
            + '{0}'.format(chr(0x2080))
        )
...
for i in range(len(x[0])):
    ...
    xNormConv.circle(
        it, x[:, i], legend_label=xNormLabels[i], color=colorList[i]
    )
...
fgMaxConv.line(
    it, f, legend_label='f', color=colorObjective, line_width=LineWidth
)
...
fgBar.extra_y_ranges = {
    'gBar': Range1d(
        start=-np.max(np.abs(g)) * 1.1, end=np.max(np.max(np.abs(g)) * 1.1)
    )
}
...
fgBar.vbar(
    top=g[0], x=gLabels, width=0.9, y_range_name='gBar', color='darkgrey'
)
fgBar.vbar(
    top=g[-1],
    x=gLabels,
...
# Layout of sheet
title = Div(
    text='<h2 style="text-align: center">Design optimization monitoring</h1>'
)
headerConvPlot = Div(
    text='<h3 style="text-align: center">Convergence plots</h2>'
)
headerBarPlot = Div(text='<h3 style="text-align: center">Current design</h2>')
headerDetails = Div(
    text='<h3 style="text-align: center">Optimization details</h2>'
)
